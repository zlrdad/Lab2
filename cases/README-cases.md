...
- **`../champions.html`:** the affordance (§8) was added to the **three** built pages that already have roster rows — **Cohere, Xanadu, Magnet Forensics**. At the time this note was first written, Waabi, 1Password and Clearpath were not yet in the roster and expanding it was an open question. **That question is resolved.** The July 8 2026 holistic coherence pass expanded the highlighted roster to 15 (adding Waabi, 1Password, Clearpath + the Nortel cautionary link) and re-tallied the §01b outcome-split stacked bar accordingly (4 stayed · 3 domestic · 4 at-risk/in-play · 1 US-capital · 3 acquired/lost = 15). The overnight July 12 2026 build then closed the remaining gap: **Wealthsimple, StackAdapt, Clio, Kinaxis and MDA Space** — all five already counted in the 15-company tally, but previously plain text on the roster — got their own `cases/` pages and `colink` wiring. **The roster is now fully linked: 15/15 + Nortel = 16/16, and `cases/` holds 19 pages** (16 domestic incl. Nortel, 3 international). _(Superseded July 12 2026 by the Ada addition below — the roster is now 16/16 + Nortel = 17/17, and `cases/` holds 20 pages: 17 domestic incl. Nortel, 3 international.)_
...
## §Addition (July 12, 2026) — Ada, the 16th champion

Carlo approved **Ada** as a discretionary 16th roster champion (flagged in the overnight-build review as the strongest unbuilt candidate — a watchlist priority already carrying a full vault deep-dive, `Themes/Case Studies/Ada — Canadian Tech Case Study.md`). One new page + roster wiring, built strictly to §§1–8 (scoped block copied verbatim from `cohere.html`; four PROV entries; figures exact to the vault study). It fills the **applied-AI** layer of the AI-champion trio alongside Cohere (frontier) and Waabi (autonomy).

| Page | Accent | Status | Verdict sibling |
|---|---|---|---|
| `ada.html` | violet | `v-stay` (roster badge **At risk** · `st-risk`; "retained / flight-exposed, US-funded" caveat carried in the hero label + verdict prose) | → `element-ai` (the applied-AI champion that was lost) |

Ada is Toronto-HQ'd, founder-led and **has not relocated**, so — like Cohere/Waabi/Xanadu/1Password — the page uses `v-stay` with the foreign-capital caveat in the label/prose, not a separate colour. On `../champions.html` its roster row sits in the **at-risk cluster** (after Waabi, before Tenstorrent) as `costudy st-risk` with badge **At risk**. Every growth-stage dollar (Series B Accel; Series C Spark/Tiger Global, US$130M at US$1.2B) was US-led; Canadian capital appears only at seed/A (Version One, Leaders Fund). The 2021 US$1.2B mark is carried as stale/unverified per the vault's own caveat.

**Tally impact — the §01b outcome chart re-tallied.** Ada adds exactly one to the **at-risk / in-play** bucket: the hand-built stacked-SVG geometry, printed numbers, caption and aria-label were recomputed from **4 stayed · 3 domestic · 4 at-risk · 1 US-capital · 3 acquired (= 15)** to **4 stayed · 3 domestic · 5 at-risk · 1 US-capital · 3 acquired (= 16)** — per-company width 50.67px → 47.5px across the 70–830 span; the `split` and `roster` PROV drawers and all "fifteen"→"sixteen" prose updated to match. The §03 missing-middle through-line (Cohere/Waabi/Xanadu/Tenstorrent, the C$2–10B tier) was **left unchanged** — Ada's ~US$1.2B (~C$1.7B) mark sits below that band.
